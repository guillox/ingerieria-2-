...
class SubastaModel
{
	...
	public function listar($dato)
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT * FROM subasta WHERE (descripcion LIKE '%$dato%' OR nombre LIKE '%$dato%') AND inactivo NOT LIKE '%*%' ");
			$stm->execute();
	...
	public function bajaSubasta($id) {
		try 
		{
			$stm = $this->pdo->prepare("DELETE FROM subasta WHERE subastaID = ?");			          
    ...
    public function listarCategoria($idCategoria,$limite)
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT * FROM subasta WHERE inactivo NOT LIKE '%*%' AND categoriaID = $idCategoria  ORDER BY subastaID DESC LIMIT $limite");
			$stm->execute();
     ...
     public function listarReportFechas($rango1,$rango2)
	{
		try
		{
			...
			$stm = $this->pdo->prepare("SELECT * FROM subasta WHERE inactivo NOT LIKE '%*%' AND fecha_inicio >= ? AND fecha_inicio <= ? AND inactivo NOT LIKE '%*%' ORDER BY fecha_inicio DESC");
